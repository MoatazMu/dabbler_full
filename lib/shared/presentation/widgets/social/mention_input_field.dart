...
enum MentionType { user, group, topic, hashtag }
...
class MentionSuggestion {
  ...
  factory MentionSuggestion.fromUser(UserModel user) {
    return MentionSuggestion(
      ...
      metadata: {'username': user.displayName, 'isVerified': false},
    );
  }
}
...
enum MentionPermission { allowed, blocked, restricted, notFound }
...
class MentionInputField extends StatefulWidget {
  ...
  final Future<MentionPermission> Function(String id, MentionType type)?
  onValidateMention;
  ...
  final Widget Function(MentionSuggestion suggestion, VoidCallback onTap)?
  suggestionBuilder;
...
class _MentionInputFieldState extends State<MentionInputField> {
      ...
      child: Column(
        ...
        children: [
          ...
          Wrap(
            ...
            runSpacing: 4,
            children: _mentions
                .map((mention) => _buildMentionChip(mention))
                .toList(),
          ),
        ],
      ),
  ...
  Widget _buildMentionChip(ExtractedMention mention) {
    return Chip(
      ...
      label: Text(mention.displayText, style: const TextStyle(fontSize: 12)),
  ...
  Widget _buildDefaultSuggestionItem(
    ...
    return Container(
      ...
      child: ListTile(
        ...
        subtitle:
            suggestion.type == MentionType.user &&
                suggestion.metadata?['username'] != null
  ...
  void _selectSuggestion(MentionSuggestion suggestion) async {
    ...
    if (widget.onValidateMention != null) {
      final permission = await widget.onValidateMention!(
        suggestion.id,
        suggestion.type,
      );
  ...
  void _removeMention(ExtractedMention mention) {
    ...
    _controller.text = newText;
    _controller.selection = TextSelection.collapsed(offset: mention.startIndex);
  ...
  Widget buildMentionRichText({
    ...
    for (final match in matches) {
      ...
      if (match.start > lastMatchEnd) {
        spans.add(
          TextSpan(text: substring(lastMatchEnd, match.start), style: style),
        );
      }
    ...
    if (lastMatchEnd < length) {
      spans.add(TextSpan(text: substring(lastMatchEnd), style: style));
    }
