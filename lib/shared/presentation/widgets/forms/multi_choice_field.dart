...
class _MultiChoiceFieldState<T> extends State<MultiChoiceField<T>>
    ...
    return Row(
      children: [
        ...
        if (widget.showClearAll)
          OutlinedButton.icon(
            onPressed: widget.enabled && _selectedValues.isNotEmpty
                ? _clearAll
                : null,
        ...
        Wrap(
          ...
          children: [
            ...
            if (widget.maxDisplayedItems != null &&
              ...
              widget.hiddenItemsBuilder?.call(
                    selectedItems.length - widget.maxDisplayedItems!,
                  ) ??
                  Chip(
                    label: Text(
          ...
          children: [
            ...
            Wrap(
              ...
              children: entry.value
                  .map((item) => _buildChoiceChip(item))
                  .toList(),
            ),
  ...
  Widget _buildChoiceChip(ChoiceItem<T> item) {
    ...
    if (item.customWidget != null) {
      return GestureDetector(
        onTap: widget.enabled && item.enabled
            ? () => _toggleSelection(item.value)
            : null,
      ...
      builder: (context, child) {
        ...
        return Transform.scale(
          ...
          child: FilterChip(
            ...
            selected: isSelected,
            onSelected: widget.enabled && item.enabled
                ? (selected) {
                    _toggleSelection(item.value);
                  }
                : null,
            ...
            selectedColor:
                widget.chipStyling.selectedColor ??
    ...
    setState(() {
      if (_selectedValues.contains(value)) {
        ...
        if (widget.selectionLimit?.maxSelections != null &&
          ...
          _validationMessage =
              widget.selectionLimit!.getValidationMessage(
      ...
      // Update validation
      _validationMessage =
          widget.selectionLimit?.getValidationMessage(_selectedValues.length) ??
          '';
    });
  ...
  void _selectAll() {
    ...
    setState(() {
      final availableItems = _filteredItems
          .where((item) => item.enabled)
          .toList();

      if (widget.selectionLimit?.maxSelections != null) {
        final maxItems = availableItems.take(
          widget.selectionLimit!.maxSelections!,
        );
...
class MultiChoicePresets {
  ...
  static MultiChoiceField<String> skillLevels({
    ...
    return MultiChoiceField<String>(
      ...
      onSelectionChanged: onChanged,
      label: label,
      chipStyling: const ChipStyling(showCheckmark: false, elevation: 1),
    );
  }
  ...
  static MultiChoiceField<String> availability({
    ...
    return MultiChoiceField<String>(
      ...
      showSelectAll: true,
      showClearAll: true,
      chipStyling: const ChipStyling(spacing: 4, runSpacing: 4),
    );
  }
}
