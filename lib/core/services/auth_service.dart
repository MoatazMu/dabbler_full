...
class AuthService {
  ...
  Future<AuthResponse> signUpWithEmailAndMetadata({
    ...
    try {
      print(
        '🔐 [DEBUG] AuthService: Signing up user with email and metadata: $normalizedEmail',
      );
      ...
      final response = await _supabase.auth.signUp(
        ...
        password: password,
        data:
            metadata, // Pass metadata so database trigger can create complete profile
      );
      ...
      print(
        '🔧 [DEBUG] AuthService: Attempting fallback signup without metadata...',
      );
      ...
      try {
        ...
        if (basicResponse.user != null) {
          print(
            '✅ [DEBUG] AuthService: Basic signup succeeded, creating profile manually',
          );
          await _ensureUserProfileExists(
            basicResponse.user!.id,
            normalizedEmail,
            metadata,
          );
        ...
        print(
          '❌ [DEBUG] AuthService: Fallback signup also failed: $fallbackError',
        );
  ...
  Future<void> _ensureUserProfileExists(
    ...
    try {
      print(
        '👤 [DEBUG] AuthService: Ensuring user profile exists for: $userId',
      );
      ...
      if (metadata['name'] == null ||
          (metadata['name'] as String).trim().isEmpty) {
        throw Exception(
      ...
      if (metadata['gender'] == null ||
          (metadata['gender'] as String).trim().isEmpty) {
        throw Exception(
      ...
      if (gender != 'male' && gender != 'female') {
        throw Exception(
          'VALIDATION ERROR: Gender must be either "male" or "female". Received: "$gender"',
        );
      }
      ...
      if (metadata['intent'] == null ||
          (metadata['intent'] as String).trim().isEmpty) {
        throw Exception(
      ...
      print(
        '✅ [DEBUG] AuthService: All mandatory fields validated successfully',
      );
      print(
        '📊 [DEBUG] display_name="$displayName", age=$age, gender=$gender, sports=$sports, intent=$intent',
      );
      ...
      print(
        '✅ [DEBUG] AuthService: User profile created successfully: $result',
      );
    } catch (e) {
      print('⚠️ [DEBUG] AuthService: Direct profile insert failed: $e');
      print(
        '❌ [DEBUG] AuthService: Failed profileData had display_name: "${metadata['name']}"',
      );

      // Try using the updateUserProfile RPC as fallback
      try {
        print(
          '🔧 [DEBUG] AuthService: Trying RPC updateUserProfile as fallback...',
        );
        final result = await updateUserProfile(
          displayName: metadata['name'] as String?,
          age: metadata['age'] as int?,
          gender: metadata['gender'] as String?,
          sports: metadata['sports'] != null
              ? List<String>.from(metadata['sports'])
              : null,
          intent: metadata['intent'] as String?,
        );
        print(
          '✅ [DEBUG] AuthService: Profile created via RPC updateUserProfile with result: $result',
        );
      } catch (rpcError) {
        print(
          '⚠️ [DEBUG] AuthService: RPC updateUserProfile also failed: $rpcError',
        );
        // Last resort - at least the auth user exists
        print(
          '⚠️ [DEBUG] AuthService: Profile creation failed, but auth user exists. User can complete profile later.',
        );

        // THROW ERROR so user knows something went wrong
        throw Exception(
          'Failed to create user profile. Please complete your profile in settings.',
        );
      }
    }
  }
  ...
  Future<void> _markPhoneAsConfirmed(String userId, String phone) async {
    try {
      print(
        '📱 [DEBUG] AuthService: Marking phone as confirmed for user: $userId',
      );
  ...
  Future<bool> checkUserExistsByEmail(String email) async {
    try {
      final normalizedEmail = _normalizeEmail(email);
      print(
        '🔍 [DEBUG] AuthService: Checking if user exists: $normalizedEmail',
      );
      ...
      try {
        ...
        if (rpcResult is bool) {
          print(
            '🔍 [DEBUG] AuthService: RPC user_exists_by_email -> $rpcResult',
          );
          return rpcResult;
        }
        ...
        print(
          '⚠️ [DEBUG] AuthService: RPC user_exists_by_email not available: ${e.message}',
        );
      } catch (_) {
        // Ignore and fallback
      }
  ...
  Future<bool> checkUserExistsByPhone(String phone) async {
    try {
      ...
      print(
        '❌ [DEBUG] AuthService: Error checking user existence by phone: $e',
      );
      return false;
    }
  }
