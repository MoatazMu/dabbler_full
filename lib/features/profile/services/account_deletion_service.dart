...
import 'image_upload_service.dart'
    show ImageUploadService, imageUploadServiceProvider;
...
class AccountDeletionService {
  ...
  Future<AccountDeletionChecklist> getPreDeletionChecklist(
    String userId,
  ) async {
    try {
      core_logger.Logger.info(
        'Generating pre-deletion checklist for user $userId',
      );
      ...
      checklist.activeGamesList = activeGames
          .map(
            (game) => {
              'id': game['id'],
              'title': game['title'],
      ...
      final recentExports = await _dataExportService.getUserExportHistory(
        userId,
      );
      ...
      core_logger.Logger.info(
        'Pre-deletion checklist generated for user $userId',
      );
      return checklist;
    } catch (e) {
      core_logger.Logger.error(
        'Error generating pre-deletion checklist for user $userId',
        e,
      );
      rethrow;
    }
  ...
  Future<void> _estimateDataVolume(
    String userId,
    AccountDeletionChecklist checklist,
  ) async {
    try {
      // Count profile data
      ...
      core_logger.Logger.warning(
        'Could not estimate data volume for user $userId',
        e,
      );
  ...
  Future<DataExportResult?> createDataBackup(
    String userId, {
    ExportFormat format = ExportFormat.json,
  }) async {
    try {
      core_logger.Logger.info(
        'Creating data backup for user $userId before deletion',
      );
      ...
      core_logger.Logger.info(
        'Data backup requested for user $userId: ${exportRequest.id}',
      );
      return null;
    } catch (e) {
      core_logger.Logger.error(
        'Error creating data backup for user $userId',
        e,
      );
      rethrow;
    }
  ...
  Future<AccountDeletionRequest> requestAccountDeletion({
    ...
    try {
      core_logger.Logger.info(
        'Starting account deletion request for user $userId',
      );
      ...
      final deletionRequest = AccountDeletionRequest(
        ...
        scheduledDeletionAt: DateTime.now().add(
      ...
      core_logger.Logger.info(
        'Account deletion scheduled for user $userId on ${deletionRequest.scheduledDeletionAt}',
      );
      return deletionRequest;
    } catch (e) {
      core_logger.Logger.error(
        'Error requesting account deletion for user $userId',
        e,
      );
      rethrow;
    }
  ...
  Future<void> cancelAccountDeletion(String userId) async {
    try {
      ...
      core_logger.Logger.error(
        'Error cancelling account deletion for user $userId',
        e,
      );
      rethrow;
    }
  ...
  Future<void> executeAccountDeletion(String userId) async {
    try {
      ...
      core_logger.Logger.error(
        'Error executing account deletion for user $userId',
        e,
      );
      ...
      try {
        ...
        core_logger.Logger.error(
          'Could not update deletion status to failed',
          updateError,
        );
      }
  ...
  Future<void> _notifyUserOfDeletionRequest(
    String userId,
  ...
  Future<List<AccountDeletionRequest>> getPendingDeletions() async {
    try {
      ...
      return response
          .map((record) => AccountDeletionRequest.fromJson(record))
          .toList();
    } catch (e) {
    ...
    try {
      ...
      for (final deletion in pendingDeletions) {
        try {
          ...
          core_logger.Logger.error(
            'Failed to delete account for user ${deletion.userId}',
            e,
          );
      ...
      core_logger.Logger.info(
        'Processed ${pendingDeletions.length} pending account deletions',
      );
    } catch (e) {
...
class AccountDeletionChecklist {
  ...
  bool get hasBlockingIssues =>
      activeGamesAsCreator > 0 || upcomingParticipations > 0;
  ...
  List<String> get warnings {
    ...
    if (upcomingParticipations > 0) {
      warnings.add(
        'You have $upcomingParticipations upcoming game participations',
      );
    }
...
class AccountDeletionRequest {
  ...
  Duration get timeUntilDeletion =>
      scheduledDeletionAt.difference(DateTime.now());
}

/// Deletion status enum
enum DeletionStatus { pending, cancelled, completed, failed }
