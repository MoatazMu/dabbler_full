...
class OnboardingGamification {
  ...
  OnboardingGamification({required SupabaseClient supabase}) : _supabase = supabase;
  ...
  Future<int> getUserPoints(String userId) async {
    try {
      ...
      return response.fold<int>(0, (sum, item) => sum + (item['points'] as int));
    } catch (e) {
  ...
  Future<void> awardPoints(String userId, int points, String source, String description) async {
    try {
  ...
  Future<List<Badge>> _checkAndAwardBadges(String userId) async {
    try {
      ...
      final badgeCriteria = <Map<String, dynamic>>[
        {'id': 'onboarding_starter', 'points': 25, 'name': 'Getting Started', 'description': 'Started your onboarding journey'},
        {'id': 'profile_complete', 'points': 50, 'name': 'Profile Complete', 'description': 'Completed basic profile information'},
        {'id': 'sports_enthusiast', 'points': 75, 'name': 'Sports Enthusiast', 'description': 'Added multiple sports preferences'},
        {'id': 'onboarding_champion', 'points': 100, 'name': 'Onboarding Champion', 'description': 'Completed entire onboarding flow'},
      ];
      ...
      for (final criteria in badgeCriteria) {
        if (points >= criteria['points']! && !existingBadgeIds.contains(criteria['id'])) {
  ...
  Achievement getStepAchievement(int step, String variant) {
    final achievements = {
      1: Achievement(
        title: variant == 'gamified' ? '🎯 Profile Created!' : 'Profile Created!',
        description: variant == 'gamified' ? 'You earned 25 points!' : 'Looking good!',
      ...
      2: Achievement(
        ...
        description: variant == 'gamified' ? 'You earned 30 points!' : 'Ready to play!',
        points: variant == 'gamified' ? 30 : 0,
        iconUrl: 'assets/achievements/sports_added.png',
      ),
      3: Achievement(
        title: variant == 'gamified' ? '📍 Preferences Set!' : 'Preferences Set!',
        description: variant == 'gamified' ? 'You earned 25 points!' : 'We\'ll find perfect games!',
        points: variant == 'gamified' ? 25 : 0,
        iconUrl: 'assets/achievements/preferences_set.png',
      ),
      4: Achievement(
        title: variant == 'gamified' ? '🔒 Privacy Secured!' : 'Privacy Settings Complete!',
        description: variant == 'gamified' ? 'You earned 20 points!' : 'Your data is protected!',
  ...
  Future<CompletionCelebration> getCompletionCelebration(String userId, String variant) async {
    final totalPoints = await getUserPoints(userId);
    final badges = await getUserBadges(userId);
    
    return CompletionCelebration(
      title: variant == 'gamified' ? '🎉 Congratulations Champion!' : '🎉 Welcome to Dabbler!',
  ...
  SpecialOffer? _getSpecialOffer(String variant) {
    if (variant == 'gamified') {
      return SpecialOffer(
        ...
        description: 'Complete your first game booking within 24 hours and get 50 bonus points!',
  ...
  Future<CompletionCelebration> generateCompletionCelebration(String userId) async {
  ...
  Future<String> getNextSuggestedAction(String userId) async {
    try {
      ...
      final suggestions = ProfileStrengthCalculator.getSuggestions(response, strength);
      return suggestions.isNotEmpty ? suggestions.first : 'Complete your profile to get started!';
    } catch (e) {
...
class Badge {
  ...
  factory Badge.fromJson(Map<String, dynamic> json) {
    return Badge(
      ...
      description: json['badge_definitions']?['description'] ?? json['description'],
...
class ProfileStrengthCalculator {
  ...
  static List<String> getSuggestions(Map<String, dynamic> profileData, int currentStrength) {
