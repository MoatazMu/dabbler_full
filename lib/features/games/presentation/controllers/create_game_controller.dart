...
class CreateGameController extends StateNotifier<CreateGameState> {
  ...
  CreateGameController({required CreateGameUseCase createGameUseCase})
  ...
  void setDateTime({DateTime? date, String? startTime, String? endTime}) {
    ...
    if (date != null &&
        date.isBefore(DateTime.now().add(const Duration(hours: 1)))) {
    ...
    state = state.copyWith(
      ...
      estimatedDuration:
          (startTime != null && endTime != null && errors.isEmpty)
          ? _timeStringToMinutes(endTime) - _timeStringToMinutes(startTime)
  ...
  void configurePlayerSettings({int? minPlayers, int? maxPlayers}) {
    ...
    if (state.selectedSport != null) {
      ...
      if (newMaxPlayers < sportLimits['min']!) {
        errors['maxPlayers'] =
            'For ${state.selectedSport}, you need at least ${sportLimits['min']} players';
      }
      if (newMaxPlayers > sportLimits['max']!) {
        errors['maxPlayers'] =
            'For ${state.selectedSport}, maximum is ${sportLimits['max']} players';
      }
    }
  ...
  Future<void> reviewAndCreate(String organizerId) async {
    ...
    try {
      final result = await _createGameUseCase(
        CreateGameParams(
