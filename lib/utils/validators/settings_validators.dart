...
class SettingsValidators {
  ...
  static Map<String, String?> validatePreferences(UserPreferences prefs) {
    ...
    errors['communication'] = validateCommunicationMethod(
  ...
  static List<String> validatePreferenceLogic(UserPreferences prefs) {
    ...
    if (prefs.preferredRadiusKm != null && prefs.maxTravelTime != null) {
      ...
      if (prefs.maxTravelTime! < estimatedTravelTime * 0.5) {
        warnings.add(
          'Travel time might be too short for selected search radius',
        );
      }
    }
    ...
    if (prefs.preferredSports.isNotEmpty &&
      ...
      final hasIndividualSports = prefs.preferredSports.any(
        (sport) => individualSports.any(
          (individual) => sport.toLowerCase().contains(individual),
        ),
      );
    ...
    if (prefs.preferredAgeRangeMin != null &&
      ...
      if (prefs.preferredSkillLevel == 'expert' &&
          prefs.preferredAgeRangeMax! < 25) {
        warnings.add(
          'Expert level players are typically older and more experienced',
        );
      }

      if (prefs.preferredSkillLevel == 'beginner' &&
          prefs.preferredAgeRangeMin! > 40) {
        warnings.add(
          'Consider allowing different skill levels for broader matching',
        );
      }
    }
    ...
    if (prefs.preferredTimes.isNotEmpty && prefs.preferredDays.isNotEmpty) {
      ...
      if (!hasWeekendDays && hasWeekdayTimes) {
        warnings.add(
          'Morning/afternoon times on weekdays might limit availability',
        );
      }
    }
  ...
  static Map<String, dynamic> getRecommendedSettings({
    ...
    if (userAge != null) {
      if (userAge < 25) {
        recommendations['preferredAgeRange'] = {
          'min': userAge - 5,
          'max': userAge + 10,
        };
        recommendations['preferredRadius'] =
            15; // Younger users might travel further
      } else if (userAge < 40) {
        recommendations['preferredAgeRange'] = {
          'min': userAge - 8,
          'max': userAge + 8,
        ...
        recommendations['preferredRadius'] =
            8; // Older users prefer closer venues
      }
    }
    ...
    switch (useCase.toLowerCase()) {
      case 'quick_match':
        if (prefs.preferredRadiusKm == null || prefs.preferredRadiusKm! > 20) {
          errors['radius'] =
              'Quick matches require smaller search radius (≤20km)';
        }
        if (prefs.maxTravelTime == null || prefs.maxTravelTime! > 45) {
          errors['travelTime'] =
              'Quick matches require shorter travel time (≤45min)';
        }
        break;

      case 'tournament':
        if (prefs.preferredGameDuration == null ||
            prefs.preferredGameDuration! < 60) {
          errors['duration'] =
              'Tournament games require longer duration (≥60min)';
        }
        if (!prefs.allowDifferentSkillLevels) {
          errors['skillLevel'] =
              'Tournaments benefit from skill level flexibility';
        }
        break;

      case 'casual_play':
        if (prefs.allowMixedGender == false &&
            prefs.allowDifferentSkillLevels == false) {
          errors['flexibility'] =
              'Casual play works better with more flexibility';
        }
        break;
    }
