...
class ProfileValidators {
  ...
  static FormFieldValidator<String> bio() {
    return (value) {
      ...
      final punctuationCount = trimmed
          .split('')
          .where((c) => '!@#\$%^&*()_+-=[]{}|;:,.<>?'.contains(c))
          .length;
  ...
  static FormFieldValidator<String> phoneNumber() {
    return (value) {
      ...
      final withoutCountryCode = cleaned.startsWith('+')
          ? cleaned.substring(1)
          : cleaned;
  ...
  static Map<String, String?> validateProfile({
    ...
    errors['username'] = ProfileValidators.username(
  ...
  static List<String> generateUsernameSuggestions(
    String? fullName, {
    int count = 5,
  }) {
    ...
    final cleanName = fullName.trim().toLowerCase().replaceAll(
      RegExp(r'[^a-z\s]'),
      '',
    );
    ...
    return suggestions
        .where(
          (s) => s.length >= 3 && s.length <= 30 && usernameRegex.hasMatch(s),
        )
        .take(count)
        .toList();
  }
