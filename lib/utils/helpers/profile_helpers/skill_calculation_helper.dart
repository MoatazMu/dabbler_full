...
class SkillCalculationHelper {
  ...
  static double calculateWeightedOverallSkill(List<SportProfile> sports) {
    ...
    for (final sport in sports) {
      final weight = sport.isPrimarySport
          ? 2.0
          : 1.0; // Primary sport gets 2x weight
  ...
  static Map<SportCategory, List<SportProfile>> groupByCategory(
    ...
    return sports.fold<Map<SportCategory, List<SportProfile>>>({}, (
      map,
      sport,
    ) {
  ...
  static bool areSkillCompatible(
    ...
    final skillDifference = (player1Sport.skillLevel - player2Sport.skillLevel)
        .abs();
  ...
  static List<SportProfile> findCompatiblePlayers(
    ...
    return candidateSports
        .where(
          (candidate) => areSkillCompatible(
            targetSport,
            candidate,
            maxSkillDifference: maxSkillDifference,
          ),
        )
        .toList();
  }
  ...
  static Map<String, dynamic> getSkillStatistics(List<SportProfile> sports) {
    ...
    return {
      ...
      'total_experience': experienceYears.fold<int>(
        0,
        (sum, years) => sum + years,
      ),
      'average_experience':
          experienceYears.fold<int>(0, (sum, years) => sum + years) /
          sports.length,
