...
class ProfileValidationHelper {
  ...
  static Map<String, dynamic> getValidationProgress(UserProfile profile) {
    ...
    final validFields = allFields
        .where(
          (field) =>
              validationResults[field] == null && _hasValue(profile, field),
        )
        .length;
  ...
  static bool _hasValue(UserProfile profile, String field) {
    switch (field) {
      ...
      case 'fullName':
        return profile.fullName?.isNotEmpty ?? false;
      case 'bio':
        return profile.bio?.isNotEmpty ?? false;
      case 'email':
        return profile.email?.isNotEmpty ?? false;
      case 'phoneNumber':
        return profile.phoneNumber?.isNotEmpty ?? false;
      case 'dateOfBirth':
        return profile.dateOfBirth != null;
      case 'location':
        return profile.location?.isNotEmpty ?? false;
      default:
        return false;
    }
  }
  ...
  static List<String> _getMissingRequiredFields(UserProfile profile) {
    ...
    if (validateUsername(profile.username) != null ||
        profile.username?.isEmpty == true) {
      missing.add('username');
    }
    if (validateFullName(profile.fullName) != null ||
        profile.fullName?.isEmpty == true) {
      missing.add('full name');
    }
    if (validateEmail(profile.email) != null ||
  ...
  static String? validateAvatarUrl(String? avatarUrl) {
    ...
    final hasValidExtension = imageExtensions.any(
      (ext) => avatarUrl.toLowerCase().contains(ext),
    );
  ...
  static Future<Map<String, dynamic>> validateUsernameWithSuggestions(
    String username,
  ) async {
    final basicValidation = validateUsername(username);

    if (basicValidation != null) {
      return {'is_valid': false, 'error': basicValidation, 'suggestions': []};
    }

    final isAvailable = await isUsernameAvailable(username);

    if (isAvailable) {
      return {'is_valid': true, 'error': null, 'suggestions': []};
    }
