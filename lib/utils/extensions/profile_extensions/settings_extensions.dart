...
extension UserSettingsExtensions on UserSettings {
    ...
    switch (type) {
      ...
      case NotificationType.systemUpdate:
        return pushNotifications ||
            emailNotifications; // System updates use both

      case NotificationType.marketing:
        return marketingEmails &&
            emailNotifications; // Marketing only via email
    }
  }
  ...
  List<String> get recommendedImprovements {
    ...
    if (marketingEmails) {
      recommendations.add(
        'Consider disabling marketing emails to reduce clutter',
      );
    }
    ...
    if (privacyLevel == 'low') {
      recommendations.add(
        'Review your privacy settings for better data protection',
      );
    }

    if (activeNotificationChannels == 0 && notificationEnabled) {
      recommendations.add(
        'Enable at least one notification channel to stay updated',
      );
    }

    if (!dataUsageOptimization && offlineMode) {
      recommendations.add(
        'Enable data usage optimization to complement offline mode',
      );
    }
  ...
  List<String> get validationIssues {
    ...
    if (notificationEnabled && activeNotificationChannels == 0) {
      issues.add(
        'Notifications are enabled but no delivery channels are active',
      );
    }

    if (smsNotifications && !pushNotifications && !emailNotifications) {
      issues.add(
        'SMS is the only notification channel - consider adding backup methods',
      );
    }

    if (offlineMode && !dataUsageOptimization) {
      issues.add(
        'Offline mode works better with data usage optimization enabled',
      );
    }
