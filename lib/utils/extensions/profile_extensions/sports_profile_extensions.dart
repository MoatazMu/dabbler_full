...
extension SportsProfileExtensions on SportProfile {
  ...
  bool get isProfessional =>
      skillLevel == SkillLevel.expert && yearsPlaying >= 5;
  ...
  double get proficiencyScore {
    ...
    final experienceScore =
        math.min(yearsPlaying, 10) / 10.0 * 5; // Scale to 0-5
  ...
  bool canCompeteWith(SportProfile other) {
    ...
    final skillDiff =
        _getSkillLevelNumeric(skillLevel) -
        _getSkillLevelNumeric(other.skillLevel);
  ...
  bool get canCoach => skillLevel == SkillLevel.expert && yearsPlaying >= 3;
  ...
  bool isTeamCompatibleWith(List<SportProfile> teammates) {
    ...
    final sameSport = teammates.every(
      (mate) => mate.sportName.toLowerCase() == sportName.toLowerCase(),
    );
    if (!sameSport) return false;

    // Check skill level balance (within 2 levels)
    final skillLevels =
        teammates.map((mate) => _getSkillLevelNumeric(mate.skillLevel)).toList()
          ..add(_getSkillLevelNumeric(skillLevel));
