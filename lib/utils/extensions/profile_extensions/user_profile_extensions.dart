...
extension UserProfileExtensions on UserProfile {
  ...
  double get averageSkillLevel {
    ...
    final totalSkill = sportsProfiles.fold<int>(
      0,
  ...
  int get totalSportsExperience {
    if (sportsProfiles.isEmpty) return 0;
    return sportsProfiles.fold<int>(
      0,
      (sum, sport) => sum + sport.yearsPlaying,
    );
  }
  ...
  String get completionMessage {
    ...
    if (isAdvancedReady)
      return 'Almost there! $profileCompletionPercentage% complete';
    if (hasBasicCompletion)
      return 'Good start! $profileCompletionPercentage% complete';
    return 'Just getting started - $profileCompletionPercentage% complete';
  }

  /// Get next completion milestone
  String get nextMilestone {
    if (profileCompletionPercentage < 30)
      return 'Reach 30% to unlock basic features';
    if (profileCompletionPercentage < 60)
      return 'Reach 60% to unlock messaging';
    if (profileCompletionPercentage < 85)
      return 'Reach 85% to unlock all features';
  ...
  List<String> get badges {
    ...
    if (sportsProfiles.isNotEmpty &&
        sportsProfiles.every((s) => s.skillLevel >= 4)) {
  ...
  bool get isBeginner =>
      ...
      sportsProfiles.every(
        (sport) => sport.skillLevel <= 2 && sport.yearsPlaying < 2,
      );
  ...
  bool get isIntermediate =>
      ...
      !isAdvanced &&
      sportsProfiles.any(
        (sport) => sport.skillLevel >= 3 || sport.yearsPlaying >= 2,
      );

  /// Check if user is considered advanced
  bool get isAdvanced => sportsProfiles.any(
    (sport) => sport.skillLevel >= 4 || sport.yearsPlaying >= 5,
  );
  ...
  List<String> get recommendedSports {
    ...
    final currentSports = sportsProfiles
        .map((s) => s.sport.toLowerCase())
        .toSet();

    if (currentSports.contains('tennis') &&
        !currentSports.contains('badminton')) {
      recommendations.add('Badminton');
    }
    if (currentSports.contains('basketball') &&
        !currentSports.contains('volleyball')) {
  ...
  bool isCompatibleWith(UserProfile other) {
    ...
    final otherSports = other.sportsProfiles
        .map((s) => s.sport.toLowerCase())
        .toSet();
